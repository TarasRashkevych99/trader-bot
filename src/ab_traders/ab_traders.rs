...
impl Trader{
    pub fn new(name:String,ttl:u32,cash:f32,markets: Vec<Rc<RefCell<dyn Market>>>)->Self{
        Self{
            ...
            lock_sells: HashMap::from([(EUR,vec![]),(USD,vec![]),(YEN,vec![]),(YUAN,vec![])]),
            lock_purchases: HashMap::from([(EUR,vec![]),(USD,vec![]),(YEN,vec![]),(YUAN,vec![])]),
        }
    }
    ...
    pub fn get_best_sell(&self)->Option<(String, GoodLabel)>{
        ...
        for (market,l) in labels.into_iter(){
            match l.into_iter().min_by(|x,y| x.quantity.total_cmp(&y.quantity)){
                Some(a)=>bestsells.push((market.clone(),a)),
                _=>()
            }
        }
        bestsells.into_iter().max_by(|(_,x),(_,y)| x.exchange_rate_buy.total_cmp(&y.exchange_rate_buy))
    }
    ...
    pub async fn log(&self,client: &reqwest::Client,time:&mut u32, kind:CustomEventKind, good_kind:GoodKind, quantity:f32, price:f32, market:String,result:bool,error:Option<String>){
        ...
        let _res=client.post("http://localhost:8000/log").json(&craft_log_event(*time, kind, good_kind, quantity, price, market, result, error)).send().await;
        *time+=1;
    }
    pub async fn post_currentGoodLabels(&self,client:&reqwest::Client){
        ...
        for (k,m) in self.markets.iter(){
            let _res=client
                .post("http://localhost:8000/currentGoodLabels/".to_string()+&k)
    ...
    pub async fn post_traderGoods(&self,client: &reqwest::Client){
        let mut body:Vec<TraderGood>=self.goods
                     .borrow()
                     .iter().map(|(kind,good)| TraderGood{ kind:*kind, quantity:good.get_qty()})
                            .collect();
    ...
    pub fn trade(&mut self,client: reqwest::Client){
        ...
        for mut i in 0..self.ttl{
            match last_lock.clone(){
                        ...
                        Ok((qty,offer,token))=>{
                            ...
                            match self.sell(market.clone(), token.clone(),k, qty){
                                Ok(good)=>{
                                    ...
                                    r.block_on(self.log(&client, &mut i, CustomEventKind::Bought, k, qty, offer, m_sell, true, None));
                                    r.block_on(self.post_traderGoods(&client));
                                    r.block_on(self.post_currentGoodLabels(&client));
                                },
                                Err(se)=>{
                ...
                a=>{
                    let (m_buy,market,rate_buy,k)=match a{
                        Some((EventKind::Sold,k))=>{
                            ...
                            (m_buy,market,rate_buy,k)
                        },
                        _=>{
                            let Some((m_buy,gl))=self.get_best_buy() else {todo!()};
                            let market=self.markets.get(&m_buy).unwrap().clone();
                            (m_buy.clone(),market,gl.exchange_rate_buy,gl.good_kind)
                        }
                    };
                    match self.get_qty(k, 0., rate_buy, EventKind::LockedBuy){
                        Some(qty)=>{
                                ...
                                Ok((qty,price,token))=>{
                                    ...
                                    match self.buy(market, token.clone(), qty){
                                        Ok(good)=>{
                                            ...
                                            r.block_on(self.log(&client, &mut i, CustomEventKind::Bought, k, qty, price, m_buy.to_string(), true, None));
                                            r.block_on(self.post_traderGoods(&client));
                                            r.block_on(self.post_currentGoodLabels(&client));
                                        }
                                        Err(be)=>{
